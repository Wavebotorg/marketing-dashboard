...
const TokenDashboard = ({ searchAdmin }) => {
...
//search
const filterData = (searchAdmin) => {
  const filterData = allAdmin?.filter((obj) => {
    const searchValues = [obj?.name?.toLowerCase(), obj?.email?.toLowerCase()]

    const searchValueString = searchValues.filter(Boolean).join(' ')

    return searchValueString.includes(searchAdmin.toLowerCase())
  })

  setFiltred(filterData)
}
useEffect(() => {
  if (searchAdmin === '') {
    setFiltred([])
  } else {
    filterData(searchAdmin)
  }
}, [searchAdmin])
...
// Check if allAdmin is an array before using slice
const clientsPaginated = Array.isArray(allAdmin)
  ? allCoinData.slice(perPage * currentPage, perPage * (currentPage + 1))
  : []

console.log(
  '🚀 ~ file: AddAdminData.tsx:109 ~ TableAddAdmin ~ clientsPaginated:',
  clientsPaginated
)
const numPages = allAdmin.length / perPage
  ...
  return (
    <div className="2xl:pl-52 xl:pl-60 md:pl-4 sm:pl-4 xsm:pl-12 mx-auto ">
      <div className="flex flex-col xl:justify-center xl:ml-16 xl:mr-12 lg:ml-2 lg:mr-5 md:ml-0 xsm:ml-5 mr-5  mt-10">
        <h1 className="text-[#191d22]   font-medium max-w-screen-lg   text-2xl  md:text-3xl  text-center md:text-left ">
